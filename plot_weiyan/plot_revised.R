...
plotSentimentVsBoxOffice <- function () {
  ...
  #p <- p +geom_text_repel(data=movieData, color = 'black',
   #                       size = defaultTextLabelSize,nudge_y=0.005, nudge_x=0.01)
  
  
  p <- p + geom_text(vjust=-1.1, hjust="inward", check_overlap=TRUE, color='black',
            size = defaultTextLabelSize)
  p <- p + geom_text(data=movieData[movieData$Name=='The Revenant',], nudge_y=1, nudge_x=0.0175,
                              color='black', size=defaultTextLabelSize)
  p
...
plotRottenTomatoesVsBoxOffice <- function () {
  p <- scatterPlot(movieData, 'rottenTomatoes', 'boxOffice', 'Name',
                   ...
                   'Total box office earnings (as of 2/13/16)')
  p <- p + scale_y_continuous(label=function(x){return(paste0("$", x / 1e6, 'M'))})
  p <- p+ #geom_text(color='black', size = defaultTextLabelSize, hjust='inward')
    geom_text(vjust=-1, hjust="inward", check_overlap=TRUE, color='black',
              size = defaultTextLabelSize)
